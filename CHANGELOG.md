...
завершены (фаза 0); поверх них поставлены EPIC-001 … EPIC-005, и заложен фундамент EPIC-006.
Приложение существует и запускается из исходников: Express 5 отдаёт `/health`, `/ready` и
`/api/v1/meta`, `prisma/schema.prisma` и две миграции создают пять таблиц с RLS,
`docs/api/openapi.yaml` — источник истины контракта, а SPA на Vite + React 19 поднимается с темой,
роутером и оболочкой. Чего нет — перечислено ниже, в разделе
[«Not yet present»](#not-yet-present); коротко: **реализации** аутентификации (схема и контракт
есть, маршрутов нет), каталогов локалей, доменных таблиц за пределами идентичности, `Dockerfile` и
образа. Поэтому
...
- Токен сброса пароля — в теле, не в query: query-строка уезжает в access-log каждого прокси и в
  `Referer`, а логи живут дольше часового TTL токена.
