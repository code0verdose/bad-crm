...
- Резолв учётной записи и сессии до того, как известна организация, — через три `SECURITY DEFINER`
...
> **Обратно совместимые миграции.** Обе миграции этого блока — expand-шаг: nullable-колонка,
> составной внешний ключ, три функции и один конкурентный индекс. Старый код о них не знает и их
...
| `MAIL_FROM` | новая, опциональная | нет | Адрес, от которого инсталляция шлёт письма; нужен, если задан `SMTP_URL` | Релей, проверяющий отправителя, отвергнет каждое письмо. Строка `warn` при старте; отказом станет в релизе, где почта подключается к контейнеру |

**Изменившееся поведение существующих переменных:** `DATABASE_URL` теперь проверяется при старте —
инсталляция, где она указывает на роль-владельца схемы или на суперпользователя, **не поднимется**.
