...
Спецификация завершена (фаза 0). **EPIC-001…EPIC-012 — в статусе `review`**: вся M1 и первая половина
M2. E2E-харнесс работает и проверен в CI — джоба `end-to-end` собирает установку целиком и гоняет
15 сценариев (вход, выход, доступность, языки, изоляция арендаторов).
...
frontmatter); его прогресс считает истории в статусе `done`, а не `review`, поэтому столбец
показывает нули — смотри статусы эпиков, а не проценты.
...
сотрудника, на справочнике и на матрице ролей. Восьмой («ловушка фокуса в модалке») **закрыт**: обе модалки
продукта покрыты — диалог офбординга (EPIC-012) и `role-matrix-preview-modal`. Второй тест монтирует
экран целиком, а не компонент: возврат фокуса идёт на **триггер**, а триггер в момент открытия
`disabled`, и стенд с обычной кнопкой доказал бы не то, что продукт отгружает.
...
| Проектные агенты-ревьюеры | 10 | `.claude/agents/` |
...
- **EPIC-010** — E2E-харнесс на Playwright поверх поднятого стека, 15 сценариев в CI, `axe` в каждом
  прогоне.
...
Десять специализированных ревьюеров. Все только читают и отчитываются — **код не редактируют**.
