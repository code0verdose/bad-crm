...
### Текущее состояние — M1, EPIC-006 почти закрыт

Спецификация завершена (фаза 0). EPIC-001…EPIC-005 — в статусе `review`. EPIC-006 остаётся
`in-progress`, и это не бухгалтерия: восемь историй из девяти переведены в `review` на зелёном
commit-гейте, а **STORY-006-09** (contract-шаг `owner_id NOT NULL` и запрет офбординга владельца без
передачи владения) не реализована — ни constraint, ни передачи владения в коде нет. По
[`rules/epic-driven-development.mdc`](rules/epic-driven-development.mdc) недоделка не переносится
хвостом, поэтому эпик закрывается вместе с ней. Актуальный борд —
...
- передавать в AI-ассистента — контекст `ai` физически не имеет порта к контексту `vault`, и это
...
Vault и всё производное от него. Технически: контекст `ai` не имеет порта к контексту `vault`
(проверяется архитектурным тестом на запрет импорта), для vault-сущностей не создаются
`EmbeddingChunk`, RAG-ретривер работает по allow-list `entityType` без `VAULT_ITEM`, компоненты
