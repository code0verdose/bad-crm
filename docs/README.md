...
**Состояние проекта — фаза 0:** документация написана, кода нет. Документы описывают то, что
спроектировано, а не то, что работает. Рабочее соглашение и инварианты — в
...
| [`architecture/adr/`](architecture/adr/) | 21 Architecture Decision Record — по одному решению на файл, с контекстом, решением и отвергнутыми альтернативами | **Перед попыткой изменить принятое решение**: сначала читаем ADR, потом спорим |
...
`api/openapi.yaml` — источник истины контракта — появится в **EPIC-003**; сейчас его нет.
...
commit-гейта); в фазе 0 каталог пуст.
...
flowchart TB
    ...
    subgraph ops["Эксплуатация"]
        ...
        run["runbooks/<br/>install · upgrade · backup · incident"]
    end
