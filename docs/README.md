...
| [`architecture/adr/`](architecture/adr/) | 25 Architecture Decision Record — по одному решению на файл, с контекстом, решением и отвергнутыми альтернативами | **Перед попыткой изменить принятое решение**: сначала читаем ADR, потом спорим |
...
| [`runbooks/audit-log.md`](runbooks/audit-log.md) | Журнал действий в эксплуатации: партиции по месяцам и команда их создания, что делать с непустой партицией DEFAULT, права трёх ролей, оценка объёма и роста, почему запись нельзя исправить | При обновлении (шаг 7d); когда журнал растёт быстрее ожидаемого; при вопросе «можно ли поправить запись» |
...
| [`runbooks/e2e.md`](runbooks/e2e.md) | Запуск и отладка Playwright: что поднять до первого прогона, установка браузеров отдельной командой, переменные окружения, чтение trace и HTML-отчёта, почему локально нет повторов и запрещены фиксированные ожидания | При работе с e2e-сценариями; когда падает весь набор сразу; при добавлении спеки |
