...
**Колонка nullable, и это expand-шаг, а не ослабление инварианта.** Организация вставляется раньше,
...
> **Открытый пункт: `ON UPDATE CASCADE` на этом ключе — не то действие** (замечено 2026-07-29).
...
> **Открытый пункт: contract-шаг.** Пока колонка nullable, «у организации всегда есть владелец» не
> держится ничем на уровне БД — только кодом регистрации. Закрывающий шаг —
> `CHECK (owner_id IS NOT NULL) NOT VALID` → `VALIDATE CONSTRAINT` → `SET NOT NULL` (порядок из
> `rules/db-migrations.mdc`, 4) — запланирован в
> [STORY-006-09](../../epics/epic-006-auth-core/stories/story-006-09-owner-integrity-contract-step.md)
> и не должен ехать в том же релизе, что и expand.
