...
2. **Разный жизненный цикл.** Увольнение — это `terminatedAt` в профиле плюс `status = SUSPENDED`
   ...
   **С STORY-012-05 обе колонки пишет одна операция** — `POST /users/{userId}/deactivate`, в одной
   транзакции вместе с инкрементом `permissions_version`, отзывом всех сессий (`revoked_reason =
   'OFFBOARDING'`) и удалением строк `team_members`. Это не деталь реализации, а свойство, на
   которое опирается прекращение доступа: `AuthenticateSessionQuery` на **каждом** запросе требует
   одновременно «сессия не отозвана», «`status = ACTIVE`» и «версия прав совпадает», поэтому
   расхождение между этими записями во времени было бы окном, в котором живой access-токен ещё
   отвечает. `terminatedAt` при этом пишется через `updateMany`: у аккаунта, принявшего приглашение
   сегодня утром, кадровой строки ещё нет, и это обычный случай, а не отсутствующая строка.

   Реактивация возвращает `status = ACTIVE` и `terminatedAt = NULL`, но **не восстанавливает
   членства**: `team_members` остаются удалёнными. Автоматическое восстановление было бы выдачей
   доступа на основании состояния, верного полгода назад.
