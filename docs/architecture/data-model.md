...
последний субъект, у которого нельзя отнять административные права, поэтому попытка удалить или
разжаловать его блокируется, пока владельцем не назначен другой пользователь (передача владения —
одна транзакция: `UPDATE organizations SET owner_id = …` + запись в `AuditLog`). Без явной колонки
«минимум один владелец» пришлось бы вычислять агрегатом по ролям при каждой проверке, и любая
ошибка в матрице прав могла бы оставить организацию без администратора вообще. Порядок создания
разрешается отложенным FK: сначала вставляется `Organization`, затем первый `User`, затем в той же
транзакции проставляется `ownerId` (`DEFERRABLE INITIALLY DEFERRED`).
