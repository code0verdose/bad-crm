...
| Mantine 7 + CSS Modules, без Tailwind | Tailwind + headless-kit; MUI; собственный дизайн-система | Готовые сложные компоненты (таблицы, дровера, даты) закрывают большую часть UI; CSS Modules — рекомендованный автором Mantine путь, нет войны каскадов | [0006](adr/0006-mantine-css-modules-no-tailwind.md) |
