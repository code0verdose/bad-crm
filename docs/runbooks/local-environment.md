...
| `pnpm stylelint` | Stylelint по всем `packages/*/src/**/*.css`: литеральные цвета, магические пиксели, физические свойства. Отдельно запускать не обязательно — задача `//#stylelint` подвешена к `lint`, поэтому `pnpm turbo run typecheck lint build test` уже её прогоняет |

Правило стилей — `rules/design-system.mdc` §3 (только токены), конфигурация — `stylelint.config.js`
в корне. Автоисправляемая часть (порядок, дубли, кавычки) чинится
`pnpm exec stylelint "packages/*/src/**/*.css" --fix`; литералы и физические свойства не
исправляются автоматически намеренно — замена подбирается по смыслу, а не механически
(`padding: 13px` → какой именно шаг шкалы имелся в виду, знает только автор).
