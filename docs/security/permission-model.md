...
**Про owner.** `isOwner` — производное свойство: у пользователя есть активное (непросроченное)
назначение системной роли `owner`. Организация обязана иметь **минимум одного** владельца —
инвариант проверяется в use-case снятия роли и передачи владения
(`organization:transfer_ownership`). Рекомендуется денормализованное `Organization.ownerId` для
дешёвой проверки «последний владелец» (см. §12).
...
| `organization:transfer_ownership` | organization | transfer_ownership | MANAGER | **да** | organization |
...
**Итого: 318 ключей, из них 102 отмечены `isDangerous`.** Числа зафиксированы тестом
...
отдельными галочками там, где он тривиально имеет всё: у него **все 318 ключей**, и это проверяется
...
- **Удалить или переименовать `key`.** `Role.key` системной роли — идентификатор в коде и в
  снапшот-тесте (§9б).
- **Снять последнего владельца.** Use-case `revoke-role` и `transfer-ownership` проверяют, что после
  операции остаётся минимум один активный `owner`, иначе `DomainError('last_owner_required')`.
