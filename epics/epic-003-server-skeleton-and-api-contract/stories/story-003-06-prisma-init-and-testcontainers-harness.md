...
- [ ] **У `pnpm db:grants` нет исполняемого места: `db:migrate` не определён ни одним пакетом.** Корневой `db:migrate` — обёртка `turbo run db:migrate`, а самой задачи `db:migrate` нет ни в одном `package.json`, поэтому гранты сегодня применяются только руками. Пропуск шага не виден ничем: миграция проходит, приложение работает, а `backup_role` не получает `SELECT` на новую таблицу — и это обнаруживается на первом `pg_dump`, то есть в день, когда бэкап понадобился. **Сделано, когда:** `@bad-crm/server` объявляет `db:migrate` как `prisma migrate deploy && pnpm db:grants` (гранты — часть команды, а не соседний пункт инструкции, который можно забыть), и [`docs/runbooks/upgrade.md`](../../../docs/runbooks/upgrade.md) содержит шаг применения грантов после миграции и после восстановления — сейчас он про этот шаг не знает вовсе. *(технический долг EPIC-001, зафиксирован 2026-07-27)*
- [ ] **`pnpm db:grants` выполняется через `docker compose exec` — паттерн, уже забракованный для `db:bootstrap`.** Окружение работающего контейнера зафиксировано в момент его создания, поэтому `exec` после правки `.env` подставит **старые** значения (`POSTGRES_DB`, пароли) и отрапортует об успехе; ровно по этой причине `db:bootstrap` переведён на `docker compose run --rm` ([`stack.md`, «Команды»](../../../docs/architecture/stack.md)). Один и тот же дефект остался во второй команде. **Сделано, когда:** `db:grants` выполняется одноразовым контейнером (`docker compose run --rm --no-deps`) либо подключается с хоста по `DATABASE_MIGRATION_URL`, и есть проверка, падающая при возврате к `exec`. *(технический долг EPIC-001, зафиксирован 2026-07-27)*
