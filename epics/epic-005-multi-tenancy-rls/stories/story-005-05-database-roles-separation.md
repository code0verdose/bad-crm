...
- [ ] **Гранты на последовательности выданы шире собственного правила.** `packages/server/prisma/sql/01-grants.sql` обходит таблицы по признаку наличия `organization_id` и раздаёт `app_user` права выборочно (партиции — `REVOKE`, журналы — только `SELECT, INSERT`, всё прочее — невидимо), но последовательности обрабатывает без разбора: `GRANT USAGE, SELECT ON SEQUENCE … TO app_user` получает **каждая** последовательность схемы `public`, в том числе принадлежащая таблице, которую `app_user` намеренно не видит (сегодня это `_prisma_migrations`, завтра — любая платформенная таблица). `USAGE` на чужой последовательности позволяет двигать её `nextval()`, `SELECT` — читать текущее значение, то есть косвенно оценивать объём чужих данных. Дефект тихий: ничего не ломается, правило просто перестаёт быть правилом. **Сделано, когда:** последовательность получает права `app_user` только если её владеющая таблица (`pg_depend` → `pg_class`) сама попала под грант; у `backup_role` `SELECT` остаётся на всех последовательностях (это требование `pg_dump`); `test/infra/grants-sql.test.ts` проверяет обе стороны правила. *(технический долг EPIC-001, зафиксирован 2026-07-27)*
