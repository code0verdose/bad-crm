...
**Закрыта 2026-07-30.** Инстанс `createI18n`, семь namespace (`common`, `validation`, `errors`,
`nav`, `auth`, `filter`, `pagination`) в EN и RU, провайдер в дереве приложения, 26 ключей переведены.

Два решения стоит знать.

**`nsSeparator: '.'` — на нём держится вся раскладка**, и оно проверено опытом, а не памятью: i18next
делит ключ по **первому** разделителю, поэтому `auth.login.title` разбирается как namespace `auth` и
ключ `login.title`, а `keySeparator` дальше идёт по вложенности. Это позволяет ключу выглядеть в коде
одним иерархическим путём — форма, которую предписывает ADR-0019, — при том что файлы разделены по
namespace, а это и есть условие ленивой загрузки с чанком маршрута.

**Тесты именуют ключи, а не текст.** Набор поднимает i18next в `cimode` с
`appendNamespaceToCIMode: true` (без него cimode возвращает ключ **без** namespace). Тест,
утверждающий «Sign in», падает, когда кто-то улучшает формулировку, — и учит следующего читателя
править утверждение не читая. Единственное исключение — `test/app/boot.test.tsx`: он про настоящую
точку входа, у которой нет шва для внедрения, и там факт подключённого перевода — часть предмета.

Попутно найдено собственное нарушение ADR-0019: `filter.remove.${id}` и `pagination.${control}`
собирали ключ в рантайме, а такой ключ нельзя найти статически — то есть парити-гейт про него молчит.
Заменены на статические ключи и интерполяцию.
