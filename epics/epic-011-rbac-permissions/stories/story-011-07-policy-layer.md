...
из списка задач: проектов не будет до EPIC-014. Остаётся `no-second-authorization-point.spec.ts` —
архитектурный тест на вторую точку вычисления прав; сегодня его роль выполняет агент
`permission-matrix-auditor` в гейте, а автоматический вариант имеет смысл, когда появятся домены с
собственными policy (M3).
