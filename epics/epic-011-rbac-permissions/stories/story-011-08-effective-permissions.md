...
- [x] **Клиентская половина, п. 11–12** (2026-08-05): юнит `units/iam` — `me/permissions` в TanStack
  Query (ключ `QueryKeys.Permissions.mine()`), хук `useCan` и компонент `<Can>`. Хук вызывает **тот
  же** `can()` из `@bad-crm/shared`, что и сервер: одна реализация, один набор табличных случаев, ни
  одного второго мнения о том, что значит право. Пока ответа нет — говорит «нельзя»: кнопка,
  появившаяся с опозданием, — мелкое неудобство, а кнопка, исчезающая под курсором, — нет.
  `<Can>` по умолчанию не показывает ничего вместо объяснения: экран со списком запрещённого —
  это карта структуры организации, выданная тому, кто смотрит.
- **Гард маршрута (п. 13)** — осмысленен вместе с первым защищённым экраном (011-10): сегодня в
  роутере нет маршрута, который стоило бы закрывать, и гард проверял бы свойство пустого множества.
