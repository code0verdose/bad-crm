...
- **`audit-partition-pruning.spec.ts` (п. 2 в части `EXPLAIN`, п. 9, п. 10) не написан.** Pruning и
  скорость вставки — свойства объёма: на пустой таблице `EXPLAIN` покажет одну партицию просто
  потому, что данных нет, а «200 событий/с» без нагрузочного сценария измеряет скорость контейнера.
  Оценка объёма и метрика описаны в `docs/runbooks/audit-log.md`; замеры — вместе с нагрузочным
  сценарием, когда журнал начнёт наполняться (EPIC-016, фаза 2).
