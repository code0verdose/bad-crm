...
describe('the entry point', () => {
  ...
  it('mounts into #root and renders the shell around the first screen', async () => {
    ...
    // Six times the default: this case boots the whole application — router, providers, i18next,
    // the shell — and takes about two seconds alone. Under `turbo run test`, with four packages'
    // suites competing for the machine, it has exceeded five, and then ten. A timeout that only
    // holds on an idle machine reports contention as a defect, which is how a suite earns the
    // reputation of being flaky; the number is deliberately far above the measured cost rather than
    // just above it, because what it guards against is the machine being busy, not the code being
    // slow. A real hang still fails — it just fails in half a minute instead of ten seconds.
  }, 30_000);
